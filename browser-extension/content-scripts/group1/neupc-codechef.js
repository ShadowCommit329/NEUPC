 ...
 * Extracts submission data from CodeChef
 */
...
(function () {
  ...
  function normalizeVerdict(verdict) {
    ...
    if (v.includes('PENDING') || v.includes('QUEUE') || v.includes('RUNNING'))
      return 'PENDING';
    if (v.includes('PARTIAL') || v.includes('PARTIALLY')) return 'PARTIAL';
  ...
  class CodeChefExtractor {
    ...
    async getUserHandle() {
      // Try multiple methods to find user handle
      ...
      // Try to find from profile dropdown or navbar
      const profileElement = safeQuery(
        '[class*="user-name"], [class*="username"]'
      );
      if (profileElement) {
        return extractText(profileElement);
      }
    ...
    async extractSubmission() {
      try {
        ...
        const submissionId =
          window.location.pathname.match(/\/complete\/([A-Z0-9]+)/)?.[1] ||
        ...
        await waitForElement(
          '.content, #primary-content, [class*="solution"]',
          3000
        ).catch(() => null);
        ...
        const timeSelectors = ['[class*="time"]', '.execution-time'];
        for (const selector of timeSelectors) {
          const el = safeQuery(selector);
          if (el) {
            const timeText = extractText(el);
            const match = timeText.match(/(\d+\.?\d*)\s*(s|ms|sec)/i);
            if (match) {
              const value = parseFloat(match[1]);
              const unit = match[2].toLowerCase();
              executionTime = unit === 'ms' ? value : value * 1000;
        ...
        // Extract memory
        let memoryUsed = null;
        const memSelectors = ['[class*="memory"]', '.memory-used'];
        for (const selector of memSelectors) {
          const el = safeQuery(selector);
          if (el) {
            const memText = extractText(el);
            const kbMatch = memText.match(/(\d+\.?\d*)\s*KB/i);
            const mbMatch = memText.match(/(\d+\.?\d*)\s*MB/i);
            if (kbMatch) {
